...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Get the user's email from localStorage (set after login)
    const email = localStorage.getItem('userEmail')
    if (email) {
      // Fetch the user's name from Supabase
      supabase
        .from('users')
        ...
        .then(({ data }) => {
          if (data) {
            ...
            localStorage.setItem('userEmail', email) // Store the email from localStorage
          }
        })
    }
  }, [])
